...
public class HtmlExtractor extends AbstractExtractor<WebContent> {
	...
	public WebContent extractContent(ReadableResource resource, ServiceLocator serviceLocator) throws IOException {
		try {
			...
			resource.makeRedirection(new URL(redirection.redirectUrl));
			return serviceLocator.getScraper().scrapContent(resource);
		}
	}
	...
	private Video scrapVideoUrl(String videoUrl, Scraper scraper) {
		try {
			return scraper.scrapContent(videoUrl, Video.class);
		} catch (Exception e) {
	...
	private Image scrapImageUrl(String imageUrl, Scraper scraper) {
		try {
			return scraper.scrapContent(imageUrl, Image.class);
		} catch (Exception e) {
